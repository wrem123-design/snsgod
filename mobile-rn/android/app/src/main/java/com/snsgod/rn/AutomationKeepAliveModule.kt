...
class AutomationKeepAliveModule(private val reactContext: ReactApplicationContext) :
  ...
  @ReactMethod
  fun areAutomationNotificationsEnabled(promise: Promise) {
    try {
      val appEnabled = NotificationManagerCompat.from(reactContext).areNotificationsEnabled()
      if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
        promise.resolve(appEnabled)
        return
      }
      val manager = reactContext.getSystemService(NotificationManager::class.java)
      val channel = manager?.getNotificationChannel(AutomationKeepAliveService.CHANNEL_ID)
      promise.resolve(appEnabled && channel?.importance != NotificationManager.IMPORTANCE_NONE)
    } catch (error: Exception) {
      promise.reject("AUTOMATION_NOTIFICATION_CHECK_FAILED", error.message, error)
    }
  }

  @ReactMethod
  fun openAutomationNotificationSettings(promise: Promise) {
    try {
      val intent = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        Intent(Settings.ACTION_CHANNEL_NOTIFICATION_SETTINGS).apply {
          putExtra(Settings.EXTRA_APP_PACKAGE, reactContext.packageName)
          putExtra(Settings.EXTRA_CHANNEL_ID, AutomationKeepAliveService.CHANNEL_ID)
        }
      } else {
      ...
      promise.reject("AUTOMATION_NOTIFICATION_SETTINGS_FAILED", error.message, error)
    }
  }

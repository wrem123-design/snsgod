...
export function LorebookScreen({ state, onBack, onChange }: {
  ...
  function addEntry() {
    setEntries(prev => [...prev, { id: makeId('lore'), title: '새 로어', keys: [], content: '', enabled: true }]);
  }
  ...
  async function save() {
    await onChange({ ...state, loreEntries: entries.map(entry => ({ ...entry, keys: entry.keys.map(key => key.trim()).filter(Boolean) })) });
    onBack();
  }
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.help}>로어북은 대화 내용에 트리거 단어가 등장할 때만 프롬프트에 주입됩니다.</Text>

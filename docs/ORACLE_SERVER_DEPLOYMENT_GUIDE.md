...
연결 뒤에는 카드에 `마지막 동기화` 시각이 표시되어야 한다. `아직 기기 등록 전` 또는 `Oracle 서버 연결 정보가 없습니다`가 표시되면 연결 키를 다시 입력해 **연결 및 동기화**를 누른다. 네트워크 요청은 20초에 종료되므로 한 번의 멈춘 요청이 이후 동기화를 영구적으로 막지 않는다.
...
Android 알림 권한은 화면 표시 권한이다. 권한이 꺼져 있어도 앱은 가능한 경우 FCM 토큰을 서버에 등록하지만 알림 배너는 표시되지 않는다. 앱의 Oracle 카드에 경고가 보이면 **알림 설정 열기**를 눌러 권한을 허용한다. 앱을 다시 열면 토큰 등록과 누락 메시지 동기화를 즉시 다시 시도한다.
